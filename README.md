...
测试目前共 **1158 条断言 / 50 个套件**（浏览器里跑；命令行因为没有 DOM 是 1047 条），
...
- **摄影机改成列表**，和镜头、灯具一样可以加多台、选一台在用。帧率和快门角度
  跟着机身走——B 机升格拍 50fps 时切过去，时间轴就按 1/100 秒算 T 档。
  有两台以上时，时间轴的拍摄参数里会多出一行「机身」可以直接切。
  最后一台不让删。旧数据里的单个 `camera` 会自动迁成只有一台的列表，已填的参数不丢。
...
非闰年的 2/29、赤道、对跖经线 ±180、北半球、极昼极夜、帧率为 0、没有镜头、
